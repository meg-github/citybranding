...
def main():
    ...
    # 解釈するテキストの処理
    st.subheader("入力テキスト")
    text = st.text_area("このシステムは入力された文章を解析し，文章に出現する単語を反映した画像を生成します．文章の中に多く出現する単語はより大きく，そうでない単語は小さく表示されます．"+"  "+"ここに解析したいテキストを入れてください．")
    ...
    # ストップワードの処理
    add_stopword = st.text_input(label="ここでは分析の対象から除く単語を設定することができます．「もの」や「する」など，日常でも頻繁に使われる単語を対象外にすることで，入力した文章の特徴を観察することができます．"+"  "+"下の枠に語を設定し，「追加」ボタンを押すことで追加できます．「削除」を押すと入力された単語がリストから削除されます．")
    ...
    st.write("現在のリスト:"+"  "+"/".join(st.session_state["stop"]))  
    # 出力結果の表示
    #st.subheader("出力結果")
